...
At the moment you are not interested in converting COMP values. Fields of type COMP are just filled with some chars that suggest that they are COMP values, e.g. "COMP-x value".

At the moment assume that the Cobol code is running on Unix or Linux, so there is no need to think about EBCDIC.
